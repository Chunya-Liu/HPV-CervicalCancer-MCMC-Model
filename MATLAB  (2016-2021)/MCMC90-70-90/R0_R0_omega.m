...
R01=zeros(length(chain(nsimu1:nsimu,:)),1);
R02=zeros(length(chain(nsimu1:nsimu,:)),1);

R0_eta=chain(nsimu1:nsimu,1);
R0_omega=chain(nsimu1:nsimu,2);
R0_u=chain(nsimu1:nsimu,3);
R0_beta_1=chain(nsimu1:nsimu,4);
R0_beta_2=chain(nsimu1:nsimu,5);
R0_sigma=chain(nsimu1:nsimu,6);
R0_theta=chain(nsimu1:nsimu,7);
R0_phi=chain(nsimu1:nsimu,8);
R0_epsilon=chain(nsimu1:nsimu,9);
R0_tau=chain(nsimu1:nsimu,10);
R0_d=chain(nsimu1:nsimu,11);

%%
for i=1:length(chain(nsimu1:nsimu,:))

R01(i)=(R0_beta_1(i)*(R0_omega(i)+mu-R0_u(i)*mu))/((R0_theta(i)+mu)*(mu+R0_eta(i)+R0_omega(i)));
R02(i)=(R0_beta_2(i)*R0_theta(i)*R0_phi(i)*(R0_omega(i)+mu-R0_u(i)*mu))/((R0_theta(i)+mu)*(R0_epsilon(i)+R0_tau(i)+mu)*( mu+R0_eta(i)+R0_omega(i) ) );
...
% 计算相关性
[R_b, P_b] = corr(R0_eta, R0_value);
[R_h, P_h] = corr(R0_eta, R01_value);
[R_a, P_a] = corr(R0_eta, R02_value);

subplot(2,2,1:2)
h = scatter(R0_eta, R0_value, 8, 'filled'); 
...
xlabel('\eta');
ylabel('R_{0}');
title(sprintf('Correlation between \\eta and R_{0}: r=%.4f (p<0.01)', R_b));
set(gca, 'FontSize', 10, 'LineWidth', 1)
box on;
grid off;
...
subplot(2,2,3)
scatter(R0_eta, R01_value,8, [1.00,0.80,0.80], 'filled');
xlabel('\eta');
ylabel('R_{I_u}');
title(sprintf('Cor. (\\eta, R_{I_u}): r=%.4f (p<0.01)', R_h));
set(gca, 'FontSize', 10, 'LineWidth', 1)
box on;
grid off;

subplot(2,2,4)
scatter(R0_eta, R02_value,10, [1.00,0.80,0.80], 'filled');
xlabel('\eta');
ylabel('R_{P}');
title(sprintf('Cor. (\\eta, R_{P}): r=%.4f (p<0.01)', R_a));
set(gca, 'FontSize', 10, 'LineWidth', 1)
box on;
grid off;
